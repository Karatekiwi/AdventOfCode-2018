...
public class Day14 {
	...
	private long calculateResult(String input) {
		...
		return sequence.substring(0, index).length() + result;
	}

	private boolean findSequence(String input) {
		int lengthOld = sequence.length();

		// we do not want to have to search the whole string everytime
		if (lengthOld > 50) {
			sequence = sequence.substring(lengthOld - 20, lengthOld);
			int lengthNew = sequence.length();
			result += (lengthOld - lengthNew);
		}
